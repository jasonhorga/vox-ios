...
enum Constants {
    ...
    enum Keyboard {
        ...
        /// 键盘等待后台守护进程返回结果超时（秒）
        /// beta.62: 10s→25s（覆盖 ASR 最坏耗时）；beta.63: 25s→30s，再覆盖默认开的智能整理多出的一次 LLM 调用，
        /// 减少「ASR+整理」偏慢时被键盘提前判超时（review M4 / 智能整理 spec）。
        /// 配套：daemon 在请求被取消后不再发布迟到结果（AudioDaemonService.processAudio），杜绝 H3 幽灵注入。
        static let resultTimeout: TimeInterval = 30.0
